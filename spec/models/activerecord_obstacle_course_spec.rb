...
describe 'ActiveRecord Obstacle Course' do
  ...
  it '12. should return all items except items: 3, 4 & 5' do
    ...
    # ------------------ Using ActiveRecord ----------------------
      items = Item.where.not(name: ["Thing 3","Thing 4","Thing 5"])
    # ------------------------------------------------------------
  ...
  it '17. returns the name of items associated with a specific order' do
    ...
    # ------------------ Using ActiveRecord ----------------------
      names = Order.order(created_at: :desc).joins(:items).limit(4).pluck(:name)
    # ------------------------------------------------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results =User.select('users.*, count(user_id) AS total_order_count').joins(:orders).group(:name)
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results =User.select('users.*, count(item_id) AS total_item_count').joins(:items).group('users.name').order(name: :desc)
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
 data = User.select("users.name as user_name").joins(:order_items).select("orders.id as order_id").select("count(order_items.id) as item_count").group("users.name, orders.id").order("users.name desc")
